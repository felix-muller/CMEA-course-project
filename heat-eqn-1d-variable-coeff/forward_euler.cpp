...
// (write your solution here)
    ...
    SparseMatrix A = createPoissonMatrix(N, a); // Create the Poisson matrix A

    // Vector for storing boundary condition values
    Eigen::VectorXd G = Eigen::VectorXd::Zero(N);

    // Time-stepping loop
    for (int k = 0; k < nsteps; k++) {
        // Update boundary conditions in vector G
        G[0] = a(h * dt) * dt * gL(time[k]) / (h * h);
        G[N - 1] = a(1 - h * dt) * dt * gR(time[k]) / (h * h);

        // Update interior points using forward Euler method
